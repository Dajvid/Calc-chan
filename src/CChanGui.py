#!/usr/bin/env python3
##
# @package CChanGui Graphical user interface for Calc-chan calculator.
#
...
##
# @brief Opens a Calc-chan help window.
#
def Napoveda():
...
##
# @brief Appends string to the end of an expression.
#
# @param text_to_add text to be appended to the end of an expression.
#
...
##
# @brief Evaluates input expression and displays the result.
#
...
##
# @brief Generates a random number in range <0, 1> and appends it to the end of expression on display.
#
def str_random():
...
##
# @brief Removes the last character from the expression on display.
#
def str_ce():
    expr = display.get(1.0, tk.END).strip()
    if "Syntax error" in expr or "Value error" in expr or "Zero division" in expr:
...
##
# @brief Clears the display entirely.
#
def str_c():
    display.delete(1.0, tk.END)

##
# @brief Switches the calculator theme from light to dark.
#
def set_dark():
    global tlacitka
...
##
# @brief Switches the calculator theme from dark to light.
#
def set_light():
    global tlacitka
...
tdel = tk.Button(hlavni, text="C", width=6, heigh=3, command=str_c)
...
troot = tk.Button(hlavni, text="√", width=6, heigh=3, command=lambda text_to_add="√" : add_to_str(text_to_add))
...
hmenu.add_command(label="Nápověda", command=Napoveda)
